...
export default function About({ id }) {
  ...
  return (
      ...
      <div id="about" className='relative min-h-screen flex flex-col md:flex-row font-inter justify-center items-center bg-speckles bg-cover text-white gap-8 md:gap-20 z-1 px-4 md:px-0' >
        <div 
          className='absolute inset-0 bg-black/20 bg-cover bg-center z-0'
        />
        <motion.div
        ref={ref}
        ...
        className='relative flex flex-col p-6 md:p-10 min-h-[70vh] w-full md:w-[50vw] z-10 order-2 md:order-1 justify-center'>
          ...
          >
            {["Fullstack Developer", "ML + Systems Engineering", "20+ Production Projects", "React / Next.js / FastAPI", "Computer Vision & Forecasting"].map((stat, i) => (
              <span key={i} className='px-4 py-2 bg-black/30 border border-white/10 rounded-full text-xs md:text-sm font-medium backdrop-blur-md text-zinc-400'>
                • {stat}
              </span>
            ))}
          </motion.div>
        ...
        {/* Image DIV */}
        <div className='border-2 z-10 border-white/10 bg-me bg-cover w-full md:w-[30vw] h-[50vh] md:h-[75vh] order-1 md:order-2 rounded-2xl hover:grayscale-0 transition-all duration-700 shadow-2xl shadow-blue-900/20'/>
      </div>
